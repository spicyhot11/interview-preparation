...
1. 脏读：读到别人未提交的数据。
2. 不可重复读：同一事务里，同一行前后两次读结果不一样，通常是别的事务 `update` 了。
3. 幻读：同一事务里，同样范围条件前后两次查，结果集条数不一样，通常是别的事务 `insert/delete` 了。
...
| `Repeatable Read` | 不会 | 不会 | SQL 标准里可能会；MySQL InnoDB 中大多避免 |
| `Serializable` | 不会 | 不会 | 不会 |

为什么 `Read Committed` 会出现不可重复读：

1. 它只保证“读到的都是已提交数据”。
2. 但每次查询都可能读到最新已提交版本。
3. 所以同一事务里两次读同一行，中间如果别人提交了更新，结果就会不一样。

为什么 `Repeatable Read` 又会和幻读扯上关系：

1. 可重复读主要解决“同一行前后读取不一致”。
2. 幻读关注的是“范围内有没有新行出现或消失”。
3. 这不是一回事，所以标准 SQL 里 `Repeatable Read` 不等于绝对没有幻读。
4. MySQL InnoDB 是通过 `MVCC + Next-Key Lock` 才把幻读控制住的。

补一句容易混淆的点：

1. InnoDB 在 `Repeatable Read` 下，常规场景基本能避免幻读。
2. 但如果同一事务里混用了快照读和当前读，前后结果可能不同。
3. 所以有些资料会说“RR 下仍可能出现幻读”，本质上多半是在讲特殊读写语义，不是说 InnoDB 平时防不住。
...
1. 让普通 `SELECT` 尽量不和写操作互相阻塞。
2. 让事务在 `RC / RR` 下读到自己该看到的版本。
3. `Repeatable Read` 下保证同一事务里的普通查询结果尽量一致。

核心关键词：

1. `undo log`：保存旧版本。
2. `Read View`：决定当前事务能看到哪些版本。
3. 可见性判断：新版本不可见时，沿版本链找旧版本。

一句理解：

“同一条记录可以有多个版本，事务不是一定读最新值，而是读对自己可见的那个版本。”

快照读和当前读：

| 类型 | 常见 SQL | 是否主要走 MVCC | 读的是什么 |
| --- | --- | --- | --- |
| 快照读 | 普通 `SELECT` | 是 | 对当前事务可见的历史版本 |
| 当前读 | `SELECT ... FOR UPDATE`、`SELECT ... FOR SHARE`、`UPDATE`、`DELETE` | 否 | 最新版本，并且通常会加锁 |

`Repeatable Read` 为什么可重复读：

1. 普通查询基于 `Read View` 读快照。
2. 同一事务里后续普通查询继续沿用这份可见性规则。
3. 即使别的事务提交了新值，本事务也还能读到旧版本。

### 7. 锁

先记结论：

1. 普通 `SELECT` 一般不主动加这种行锁，更多走 `MVCC`。
2. 修改类语句、锁定读属于当前读，会加锁。
3. InnoDB 行锁本质上锁的是索引记录，不是脱离索引单独锁某个“物理表行”。

常见自动锁：

1. `Record Lock`：记录锁，锁具体索引记录。
2. `Gap Lock`：间隙锁，锁索引间隙，防止插入。
3. `Next-Key Lock`：临键锁，`记录锁 + 间隙锁`，防幻读。
4. `Insert Intention Lock`：插入意向锁，`INSERT` 前自动声明“我想往这个间隙插数据”。
5. `IS / IX`：意向共享锁、意向排他锁，表级，用来协调行锁和表锁。

我们能在 SQL 里主动触发的锁：

| SQL/语义 | 常见锁语义 | 说明 |
| --- | --- | --- |
| `SELECT ... FOR SHARE` | 共享锁 `S` | 我读这条数据，但不希望别人改它 |
| `SELECT ... LOCK IN SHARE MODE` | 共享锁 `S` | 老写法，语义同上 |
| `SELECT ... FOR UPDATE` | 排他锁 `X` | 我读完可能马上要改，不希望别人先改 |
| `UPDATE ...` | 排他锁 `X` | 修改命中的记录 |
| `DELETE ...` | 排他锁 `X` | 删除命中的记录 |
| `LOCK TABLES ... READ` | 表读锁 | 手动锁整张表读 |
| `LOCK TABLES ... WRITE` | 表写锁 | 手动锁整张表写 |

共享锁和排他锁：

1. 共享锁 `S`：大家都能读，但别人不能改。
2. 排他锁 `X`：我能读能改，别人不能再拿冲突锁。

补一句：

1. 插入意向锁是 InnoDB 在 `INSERT` 时自动加的，开发者不能手写。
2. 如果 SQL 不走索引，行锁可能锁得不精确，范围会变大。
